...
library(leaps)
# Used to perform 'Best Subset Selection'--->regsubsets()
...
par( mfrow = c(2,2))
plot( reg.summary$rss ,xlab = "No. of Var.", ylab = "RSS", type = "b")
...
# Similarly for Cp************************
plot( reg.summary$cp ,xlab="No. of Variables",ylab ="Cp",type="b",pch =20)
for_best=which.min(reg.summary$cp)
points(for_best , reg.summary$cp[for_best],col ="red",cex =2,pch=20)

# Similarly for BIC**********************
plot( reg.summary$bic ,xlab="No. of Variables",ylab ="BIC",type="b")
...
plot(val.errors, type = "b" ,pch =20)

# Finding Best model Coefficients
coeff = coef(regfit.best , which.min( val.errors ))

# Summarizing above step for prediction
# Because we donot have any pridiction function for regsubsets
...
# Ist--> Creating a vector that allocated each obsevation to one of K Folds
#IInd --> Now, We Create a matrix in which we will store the result
k = 10
set.seed(1)
folds = sample( 1:k, nrow(Hitters), replace = T)
cv.errors = matrix(data = NA,nrow = k,ncol = 19, dimnames = list( NULL, paste( 1:19)))
  
# Create a for loop that performs C.V
for( j in 1:k ) 
{
  best.fit = regsubsets( Salary~. ,data = Hitters[ folds != j, ], nvmax =19)
# fitted model on N-1 folds / groups
  for( i in 1:19)
  {
    pred = predict.regsubsets( best.fit ,Hitters[folds == j, ], id = i)
# predicting model on remaining group / fold    
    cv.errors[j , i] = mean((Hitters$Salary[ folds == j] - pred )^2 )
  }
}
# this has given 10x19 matrix of which,
# (i,j)th element corresponds to test MSE for :
# ith -->CV fold for :
# best jth variable model

mean.cv.errors = apply( cv.errors ,2 , mean) # 2 indicates function( "mean" ) will be 
                                              # applied along "column"
# ploting
